...
```
Phase 1: データ収集 [scripts/ — 決定的処理]
  collect_transcripts.py → transcripts.json
  collect_skills.py → skill-manifest.json
       ↓
Phase 2: ルーティング監査 [Sub-agent — 並列]
  routing-analyst × N batches → batch-audit-*.json
  コーディネーターがマージ → audit-report.json
       ↓
Phase 3: ポートフォリオ分析 [Sub-agent — 逐次]
  portfolio-analyst → portfolio-analysis.json
       ↓
Phase 4: 改善提案 [Sub-agent — 逐次]
  improvement-planner → improvement-proposals.json
       ↓
Phase 5: レポート生成 [scripts/ — 決定的処理]
  generate_report.py → skill-audit-report.html
       ↓
Phase 6: パッチ適用 [Human-in-the-loop]
  ユーザー承認 → apply_patches.py
```
...
実際の環境では82プロジェクトが11バッチに圧縮され、11個のサブエージェントを並列実行しています。

### ルーティング正誤の判定方法

スキルが「正しく発火した」かどうかには正解ラベルがありません。ユーザーは自分の意図にラベルを付けてくれません。

そこで、LLM-as-Judge（LLMを審判として使う）アプローチを採用しています。Anthropicの[Demystifying evals for AI agents](https://www.anthropic.com/research/demystifying-evals-for-ai-agents)でも推奨されている方法です。
...
LLM-as-Judgeの精度を担保するために、いくつかの工夫をしています。
...
|---|---|
| 正解ラベルがない（unsupervised） | LLM-as-Judgeで文脈から判定。構造化ルブリックとconfidence付き |
...
Anthropicのevals記事が "You won't know if your graders are working well unless you read the transcripts and grades from many trials" と言っているように、結局はトランスクリプトを読むことが出発点です。skill-auditorはその「トランスクリプトを読んでパターンを見つける」という人間の作業を、Agent-as-a-Judgeパターンで自動化しているわけです。
...
ポートフォリオ全体の平均ルーティング精度は **0.685** でした。前回（symlink問題修正前）の0.42から改善していますが、まだ改善の余地があります。主な問題をいくつか紹介します。
