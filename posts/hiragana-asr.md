...
$$\mathcal{L} = \mathcal{L}_{\text{CR-CTC}}(\text{kana}) + 0.3 \cdot \mathcal{L}_{\text{CTC}}(\text{phoneme})$$

CR-CTC部分はさらに分解すると以下の通りです（[Appendix C](#c-cr-ctcの動作原理) 参照）。

$$\mathcal{L}_{\text{CR-CTC}} = \mathcal{L}_{\text{CTC}} + 0.1 \cdot \frac{1}{T} \sum_{t=1}^{T-1} \text{KL}(P_t \| P_{t+1})$$
...
ここで $\boldsymbol{\pi}$ はblankを含むアライメントパス、$\mathcal{B}^{-1}$ はCTCの縮約関数の逆写像です。重要なのは $\prod P(\pi_t | \mathbf{x})$ の部分で、各タイムステップの出力確率はエンコーダの出力 $\mathbf{x}$ のみに条件付けされ、他のタイムステップの出力には依存しません。

これにより、入力に存在しない内容を「生成」するメカニズムがアーキテクチャ上存在しないのです。attention decoderが $P(y_t | y_{<t}, \mathbf{x})$ と過去の出力に条件付けされるのとは本質的に異なります。
...
CR-CTC の損失関数は[モデルアーキテクチャ](#モデルアーキテクチャ)で示した通りです。隣接フレーム間のKLダイバージェンスを最小化することで、CTC出力がスパイク的にならず滑らかな分布を保つよう正則化します。[Yao et al. (2025)](https://arxiv.org/abs/2410.05101) は複数のaugmented viewからの一貫性も含めたより包括的な定式化を提案していますが、本実装では隣接フレーム間の一貫性のみ使用しています。
