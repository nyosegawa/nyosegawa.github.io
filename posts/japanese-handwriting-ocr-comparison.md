---
title: "日本語の手書きメモを書き起こせるOCRを探すために23モデルを片っ端から試した話"
description: "手書きメモの電子化がつらいので、Claude, Gemini, GPTからHunyuanOCR, Nemotron OCR v2まで23モデルを3指標で比較しました。結論: Gemini 3.1 Proが最強です"
date: 2026-03-17
...
今日は日本語の手書きメモをいい感じに書き起こしてくれるOCRを探して、23モデルを片っ端から比較してみた話をまとめていきたいと思います。
...
APIで呼べるモデル10個と、GPU上で動かすOSSモデル13個を比較しました。
...
- 日本語手書きメモのOCRに使えるモデルを探して23モデルを比較しました
...
結果はNLS 0.589で14位。Claude 4.5 Sonnet (0.640) より下、Nanonets-OCR-s (0.557) よりは上という位置づけです。専用OCR APIとしてはGoogle Cloud Vision (0.820) やAzure AI Vision (0.830) に大きく差をつけられています。
...
結果はNLS 0.706で12位。GPT-5.4 (0.714) とほぼ同水準で、HunyuanOCR (0.698) を僅差で上回っています。
...
結果はNLS 0.718で11位。olmOCR-2 (0.723) のすぐ下、GPT-5.4 (0.714) とほぼ同水準です。
...
結果はNLS 0.413で19位。DeepSeek-OCR (0.446) の下、PaddleOCR (0.353) の上という位置づけです。日本語対応とはいえ活字・印刷文書向けの設計なので、手書きはやはり守備範囲外のようです。
