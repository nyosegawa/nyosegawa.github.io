...
具体的には、関係者間のイメージを揃える、実装との乖離を検出する基準線になる、正しい挙動の拠り所になる、といった役割があります。要件を定義する、判断の経緯を残す、外部制約を記録する、新メンバーがキャッチアップする。どれもこうした役割のバリエーションです。
...
Agentが扱いやすいドキュメントには、決定論的に検証可能であること、あるいは不変であることのいずれかの性質があると感じています。検証可能であればフィードバックループを閉じられるし、不変であればそもそも腐敗しません。逆に、コードから導出できるものはそもそも書く必要がなく、どちらの性質も持たないものだけが自然言語ドキュメントとして残ります。
...
[You Don't Need a CLAUDE.md](https://dev.to/byme8/you-dont-need-a-claudemd-jgf)の記事では約30行のエントリーポイントとしてCLAUDE.mdを機能させ、詳細はdocs/以下に分散する構成を推奨しています。これはContext効率の観点から合理的です。自分のプロジェクトではもう少し余裕を持たせて60行を上限にしていますが、短いほど良いという方向性は同じです。
...
ここまでで何を書くかとどこに置くかの話をしてきましたが、書いたドキュメントが腐らないようにするにはどうするか。わたしがいま試しているのは、ドキュメントにもコードと同じようにharnessをかけて、フィードバックループを（不完全ながら）閉じるというアプローチです。
...
コードのpre-commit hookと同じ位置づけで、ドキュメントが古くなったらコミットできないという擬似的なフィードバックループを作れます。`--no-verify` の禁止もここに入れておくと、Agentがこのループを迂回するのを防げます。

ただし、これは時間経過で腐敗の可能性を警告しているだけです。実際に腐敗しているか（＝Agentの行動を悪化させているか）は判定できません。
...
現状はlast-validatedを手動で更新していますが、理想的にはauditの結果に基づくべきだと考えています。「このドキュメントはまだ有効に機能している」とdocs-auditorが判定したときにlast-validatedが更新される、という流れが最も健全だと思っています。

## 人間のためのドキュメントはどうするか

ここまではAgentが読むドキュメントの話をしてきましたが、人間向けのドキュメント（オンボーディングガイド、運用手順書、ユーザーマニュアルなど）はどうしたらいいのでしょうか。

ひとつのアイデアとして、人間向けドキュメントはリポジトリの走査可能範囲の外に出すという選択肢があると思っています。ConfluenceでもNotionでもいいし、別のリポジトリで管理でもよいかもしれません。Agent向けドキュメントとの混在を避けることで、それぞれの最適化に集中できます。

ただし課題もあります。コード変更に連動して外部ドキュメントを自動更新する仕組みがないと、結局は乖離が生まれます。hookでドキュメント更新を促すフローも考えられますが、コード変更とドキュメント更新を同時並行で行うのは現状難しい。ここはまだ試行錯誤中です。

## まとめ

- ドキュメントの役割（関係者間のイメージを揃える、実装との乖離を検出する基準線、正しい挙動の拠り所）はCoding Agent時代でも変わらないが、フィードバックループがないと機能し続けられない
