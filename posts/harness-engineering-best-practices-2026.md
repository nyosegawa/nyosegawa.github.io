...
![ハーネスエンジニアリングの全体像](/img/harness-engineering-best-practices-2026/harness-overview.png)
...
3. エラーが検出されれば`additionalContext`としてエージェントのコンテキストに注入される
...
![Hooksフィードバックループ](/img/harness-engineering-best-practices-2026/hooks-feedback-loop.png)

### 4つのHookパターン

- Safety Gates(PreToolUse): 破壊的コマンド(`rm -rf`、`drop table`)のブロック、機密ファイル(`.env`)の編集禁止。Exit 2でブロックし、stderrの理由がエージェントにフィードバックされる
- Quality Loops(PostToolUse): ファイル編集後のリンター・フォーマッター・テスト自動実行。結果をadditionalContextとして注入し、エージェントの自己修正を駆動する
- Completion Gates(Stop): エージェントが「完了」を宣言した際のテスト実行による検証。テストが通るまでエージェントを止めさせない。ただし`stop_hook_active`フラグをチェックして無限ループを防ぐこと
...
![フィードバック速度の階層](/img/harness-engineering-best-practices-2026/feedback-speed-layers.png)
...
![E2Eテストツール選定](/img/harness-engineering-best-practices-2026/e2e-test-tools.png)
...
[Electron MCP Server](https://github.com/kanishka-namdeo/electron-mcp)はPlaywright経由でElectronアプリをテストします。34以上のMCPツールでアプリライフサイクル管理、CDP接続、要素操作、スクリーンショット撮影が可能です。

#### Tauriアプリ

TauriはWebDriverインターフェースをWindows/Linuxでサポートしています(tauri-driver経由)。WebdriverIOまたはSeleniumでE2Eテスト可能です。Tauri 2.0が2024年末にリリースされ2025-2026年で35%の採用増を記録しています。

#### ネイティブデスクトップアプリ

[TestDriver.ai](https://testdriver.ai/)はコンピュータ使用(Computer-Use)エージェントによるE2Eテストです。Web、モバイル、デスクトップ、VS Code拡張、Chrome拡張、OAuthフロー等のテスト困難な領域をカバーします。AIビジョンで画面を理解しマウス・キーボードを操作します。

#### プラットフォーム別アクセシビリティAPI

| プラットフォーム | アクセシビリティAPI | エージェント向けツール |
|------------------|---------------------|------------------------|
| macOS | NSAccessibility / AXUIElement | 成熟したツールはまだ少ない |
| Windows | UIAutomation | 成熟したツールはまだ少ない |
| Linux | AT-SPI2 (pyatspi) | 成熟したツールはまだ少ない |
| Electron (全OS) | Chromium Accessibility | Electron MCP Server |
...
![ユニバーサルE2Eパターン](/img/harness-engineering-best-practices-2026/universal-e2e-pattern.png)
...
![Codex vs Claude Code アーキテクチャ比較](/img/harness-engineering-best-practices-2026/codex-vs-claude-code.png)
...
![MVHロードマップ](/img/harness-engineering-best-practices-2026/mvh-roadmap.png)
...
- [Electron MCP Server](https://github.com/kanishka-namdeo/electron-mcp)
